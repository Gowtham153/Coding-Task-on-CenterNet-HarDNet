...
class CocoFilter():
    ...
    def _filter_categories(self):
        ...
        print('category_set',self.category_set)
        ...
        new_id = 1
        self.prev_item = 'person'
        for key, item in self.categories.items():
            #if item['name'] == car or truck: 
            #    item['name'] == Vehicle
            #print('item', item)
            #print('--------------1----------',self.prev_item)
            if item['name'] in self.filter_categories:
                
                if item['supercategory'] != self.prev_item:
                    #new_id = new_id
                #else:
                    new_id  = new_id+1
                self.new_category_map[key] = new_id
                #print('new cat', self.new_category_map)
                self.prev_item = item['supercategory']    
            #print('--------------2----------',self.prev_item)    
        print('new cat', self.new_category_map)            
        self.new_categories = []
    ...
    def main(self, args):
        ...
        #self.filter_supercategories = args.supercategories
...
if __name__ == "__main__":
    ...
    #parser.add_argument("-d", "--supercategories", nargs='+', dest="supercategories",
    #    help="List of supercategory names separated by spaces, e.g. -c person dog bicycle")
